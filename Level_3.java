...
public class Level_3 implements Screen {
    ...
    private Body leftWoodBody2;
    private Body rightWoodBody2;
    private Body topWoodBody2;
    private Body pigBodyAboveWood2;
    ...
    public void show() {
        ...
        Gdx.input.setInputProcessor(new InputAdapter() {
            public boolean touchDown(int screenX, int screenY, int pointer, int button) {
                ...
                if (!Level_3.this.isBirdLaunched && Level_3.this.currentBirdIndex < Level_3.this.birdBodies.length) {
                    Level_3.this.launchBird((float)screenX / 100.0F, (float)screenY / 100.0F);
                    return true;
                } else {
                    return false;
                }
            ...
            public boolean keyDown(int keycode) {
                if (keycode == Input.Keys.ESCAPE) {
                    ...
                    Level_3.this.goBackToPreviousScreen();
                    return true;
                } else if (keycode == Input.Keys.SPACE) {
                    // Restart the level when the spacebar is pressed
                    Level_3.this.restartLevel();
                    return true;
                }
    ...
    private void restartLevel() {
        ...
        // Reset other game objects, such as pigs, woods, etc., if necessary.
    ...
    private void createBirdBodies() {
        ...
        this.birdBodies[2] = this.createCircleBody(2.75F, 0.8F, 0.375F, BodyType.DynamicBody);
    }

    private void createWoodBodiesWithGravity() {
        // Original wooden bodies and pig bodies
        ...
        // Duplicate wooden bodies and pig bodies placed above the original ones
        this.leftWoodBody2 = this.createRectangleBody(9.1F, 3.5F, 0.35F, 1.75F, BodyType.DynamicBody);
        this.rightWoodBody2 = this.createRectangleBody(10.5F, 3.5F, 0.35F, 1.75F, BodyType.DynamicBody);
        this.topWoodBody2 = this.createRectangleBody(9.8F, 4.6F, 2.0F, 0.3F, BodyType.DynamicBody);
        this.pigBodyAboveWood2 = this.createCircleBody(9.8F, 5.3F, 0.35F, BodyType.DynamicBody);
    }
    ...
    private void drawBirds() {
        for (int i = 0; i < this.birdBodies.length; ++i) {
            if (i >= this.currentBirdIndex) {
                // Set color based on the bird index
                if (i == 0) {
                    this.shapeRenderer.setColor(1.0F, 0.0F, 0.0F, 1.0F); // Red for the first bird
                } else if (i == 1) {
                    this.shapeRenderer.setColor(0.0F, 0.0F, 1.0F, 1.0F); // Blue for the second bird
                } else {
                    this.shapeRenderer.setColor(1.0F, 1.0F, 0.0F, 1.0F); // Yellow for the third bird
                }
                // Draw the bird
    ...
    private void drawWoodBlocks() {
        ...
        this.drawRectangle(this.topWoodBody, 200.0F, 30.0F);

        // Duplicate wooden blocks
        this.drawRectangle(this.leftWoodBody2, 35.0F, 175.0F);
        this.drawRectangle(this.rightWoodBody2, 35.0F, 175.0F);
        this.drawRectangle(this.topWoodBody2, 200.0F, 30.0F);
    }
    ...
    private void drawPig() {
        ...
        this.drawCircle(this.pigBodyAboveWood2, 37.5F);
    }

    private void drawCatapult() {
        this.shapeRenderer.setColor(0.5F, 0.25F, 0.0F, 1.0F);
        this.drawRectangle(this.catapultBody, 52.5F, 125.0F);
    }
